package com.alchitry.labs2.hardware.pinout

import com.alchitry.labs2.parsers.acf.types.Artix7IoStandard

enum class PtV2TopPin(override val fpgaPin: String, override val bank: Int) : Pin {
    A3("AB22", 14),
    A5("AB21", 14),
    A9("E3", 35),
    A11("F3", 35),
    A15("M2", 35),
    A17("M3", 35),
    A21("J6", 35),
    A23("K6", 35),
    A27("M5", 35),
    A29("M6", 35),
    A33("P4", 35),
    A35("P5", 35),
    A39("G4", 35),
    A41("H4", 35),
    A45("J4", 35),
    A47("K4", 35),
    A51("P1", 35),
    A53("R1", 35),
    A57("B2", 35),
    A59("C2", 35),
    A63("F1", 35),
    A65("G1", 35),
    A69("H5", 35),
    A71("J5", 35),
    A75("J1", 35),
    A77("K1", 35),
    A4("AB18", 14),
    A6("AA18", 14),
    A10("N2", 35),
    A12("P2", 35),
    A16("L1", 35),
    A18("M1", 35),
    A22("D2", 35),
    A24("E2", 35),
    A28("L4", 35),
    A30("L5", 35),
    A34("N5", 35),
    A36("P6", 35),
    A40("G3", 35),
    A42("H3", 35),
    A46("K3", 35),
    A48("L3", 35),
    A52("N3", 35),
    A54("N4", 35),
    A58("A1", 35),
    A60("B1", 35),
    A64("D1", 35),
    A66("E1", 35),
    A70("G2", 35),
    A72("H2", 35),
    A76("J2", 35),
    A78("K2", 35),

    B3("R14", 14),
    B5("P14", 14),
    B9("R17", 14),
    B11("P16", 14),
    B15("W17", 14),
    B17("V17", 14),
    B21("AB20", 14),
    B23("AA19", 14),
    B27("Y19", 14),
    B29("Y18", 14),
    B33("AB10", 13),
    B35("AA9", 13),
    B39("W12", 13),
    B41("W11", 13),
    B45("V14", 13),
    B47("V13", 13),
    B51("W10", 13),
    B53("V10", 13),
    B57("AB12", 13),
    B59("AB11", 13),
    B63("AA11", 13),
    B65("AA10", 13),
    B69("AB13", 13),
    B71("AA13", 13),
    B75("AA14", 13),
    B77("Y13", 13),
    B4("R16", 14),
    B6("P15", 14),
    B10("P17", 14),
    B12("N17", 14),
    B16("T18", 14),
    B18("R18", 14),
    B22("V19", 14),
    B24("V18", 14),
    B28("V20", 14),
    B30("U20", 14),
    B34("AB15", 13),
    B36("AA15", 13),
    B40("Y12", 13),
    B42("Y11", 13),
    B46("V15", 13),
    B48("U15", 13),
    B52("AB17", 13),
    B54("AB16", 13),
    B58("AA16", 13),
    B60("Y16", 13),
    B64("T15", 13),
    B66("T14", 13),
    B70("Y14", 13),
    B72("W14", 13),
    B76("W16", 13),
    B78("W15", 13),

    C29("U18", 14),
    C31("U17", 14),
    C33("V2", 34),
    C35("U2", 34),
    C30("Y1", 34),
    C32("W1", 34),
    C34("M5", 14),
    C36("M6", 14),

    LED0("P19", 14),
    LED1("P20", 14),
    LED2("T21", 14),
    LED3("R19", 14),
    LED4("V22", 14),
    LED5("U21", 14),
    LED6("T20", 14),
    LED7("W20", 14),
    RESET("N15", 14),
    CLOCK("W19", 14),
    VP("L10", 0),
    VN("M9", 0),

    USB_RX("AA20", 14),
    USB_TX("AA21", 14),
    USB_D2("W21", 14),
    USB_D3("W22", 14),
    USB_D4("Y21", 14),
    USB_D5("Y22", 14),
    USB_D6("F15", 16),
    USB_D7("U7", 34),
    USB_RXF("F21", 16),
    USB_TXE("T3", 34),
    USB_RD("T16", 13),
    USB_WR("U16", 13),
    USB_SIWUI("Y17", 13),

    SPI_D0("P22", 14),
    SPI_D1("R22", 14),
    SPI_D2("P21", 14),
    SPI_D3("R21", 14),
    SPI_SCK("L12", 0),
    SPI_SS("T19", 14),

    DDR_DQ0("J22", 15),
    DDR_DQ1("M21", 15),
    DDR_DQ2("L21", 15),
    DDR_DQ3("J20", 15),
    DDR_DQ4("H20", 15),
    DDR_DQ5("G20", 15),
    DDR_DQ6("J21", 15),
    DDR_DQ7("H19", 15),
    DDR_DQ8("H13", 15),
    DDR_DQ9("G18", 15),
    DDR_DQ10("J15", 15),
    DDR_DQ11("H17", 15),
    DDR_DQ12("G15", 15),
    DDR_DQ13("G17", 15),
    DDR_DQ14("G16", 15),
    DDR_DQ15("H15", 15),
    DDR_DQS0_P("K21", 15),
    DDR_DQS0_N("K22", 15),
    DDR_DQS1_P("J14", 15),
    DDR_DQS1_N("H14", 15),
    DDR_DM0("H22", 15),
    DDR_DM1("G13", 15),
    DDR_ODT("M17", 15),
    DDR_RESET("J19", 15),
    DDR_BA0("K19", 15),
    DDR_BA1("N20", 15),
    DDR_BA2("M20", 15),
    DDR_CK_P("K17", 15),
    DDR_CK_N("J17", 15),
    DDR_CKE("M22", 15),
    DDR_CS("N19", 15),
    DDR_RAS("L20", 15),
    DDR_CAS("N22", 15),
    DDR_WE("L19", 15),
    DDR_A0("K14", 15),
    DDR_A1("M15", 15),
    DDR_A2("N18", 15),
    DDR_A3("K16", 15),
    DDR_A4("L14", 15),
    DDR_A5("K18", 15),
    DDR_A6("M13", 15),
    DDR_A7("L18", 15),
    DDR_A8("L13", 15),
    DDR_A9("M18", 15),
    DDR_A10("K13", 15),
    DDR_A11("L15", 15),
    DDR_A12("M16", 15),
    DDR_A13("L16", 15);

    companion object : PinConverter {
        override val boardSide = BoardSide.TOP
        override val version = ConverterVersion.V2
        override val standards = Artix7IoStandard.entries
        override fun acfToPin(name: String): PtV2TopPin? = entries.firstOrNull { it.name == name }

        override fun bankToVcco(bank: Int): List<String> = when (bank) {
            0 -> emptyList()
            15 -> listOf("1.35")
            14, 16, 34, 35 -> listOf("3.3")
            13 -> listOf("3.3", "2.5", "1.8")
            else -> error("Unknown bank: $bank")
        }
    }
}