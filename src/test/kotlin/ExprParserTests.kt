...
internal class ExprParserTests {
    @Test
    fun testNumbers() {
        var test = SimpleLucidTester("5b11011", ParseStage.ModuleInternals)
        ...
        test = SimpleLucidTester("hFE01", ParseStage.ModuleInternals)
        ...
        test = SimpleLucidTester("8hFFF", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("152", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("0", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("1", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("20d12", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    @Test
    fun testAddition() {
        val test = SimpleLucidTester("5b1101 + 4b0010", ParseStage.ModuleInternals)
    ...
    @Test
    fun testConcat() {
        var test = SimpleLucidTester("c{b1101, b0010, 0}", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        assertEquals(
        ...
        test = SimpleLucidTester("c{{b1101}, b0010, 0}", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("c{{b1101}, {b0010}, {4b0}}", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        assertEquals(
            ArrayValue(
                listOf(
    ...
    @Test
    fun testDup() {
        var test = SimpleLucidTester("2x{0}", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("8x{2b10}", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        assertEquals(
        ...
        test = SimpleLucidTester("{8}x{2b10}", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        assertEquals(null, test.context.expr.resolve(tree))
        assert(test.hasErrors)
        assert(test.hasNoWarnings)
    ...
    @Test
    fun testArray() {
        var test = SimpleLucidTester("{0}", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        assertEquals(
            ArrayValue(
                listOf(
        ...
        test = SimpleLucidTester("{0, 2b10, 2b11}", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        assertEquals(null, test.context.expr.resolve(tree))
        assert(test.hasErrors)
        assert(test.hasNoWarnings)
        ...
        test = SimpleLucidTester("{b00,b01,b10}", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        assertEquals(
            ArrayValue(
                listOf(
    ...
    @Test
    fun testMultiply() {
        var test = SimpleLucidTester("20 * 40", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("20 * {40}", ParseStage.ModuleInternals)
        ...
        test = SimpleLucidTester("\$signed(20) * 40", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    @Test
    fun testDivide() {
        var test = SimpleLucidTester("40 / 8", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("40 / 5", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    @Test
    fun testShift() {
        var test = SimpleLucidTester("40 >> 3", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("-8 >> 2", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("8 << 1", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    @Test
    fun testBitwise() {
        var test = SimpleLucidTester("b1101 & b1001", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("b001101 & b1001", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$signed(b001101) & \$signed(b1001)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("b1101 | b1001", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("b101101 | b1010", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    @Test
    fun testReduction() {
        var test = SimpleLucidTester("|b1001", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("|b0000", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("&b1001", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("&b1111", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("&b1x01", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("^b1011", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    @Test
    fun testCompare() {
        var test = SimpleLucidTester("10 < 4", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("10 > 4", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("4 >= 10", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("10 >= 10", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    @Test
    fun testLogical() {
        var test = SimpleLucidTester("10 || 0", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("0 || 0", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        assert(test.hasNoIssues)
        ...
        test = SimpleLucidTester("10 && 0", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        assert(test.hasNoIssues)
    ...
    @Test
    fun testTernary() {
        var test = SimpleLucidTester("10 ? 1 : 2", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        assert(test.hasNoIssues)
        ...
        test = SimpleLucidTester("10b0 ? 1 : 2", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        assert(test.hasNoIssues)
    ...
    @Test
    fun testInvert() {
        var test = SimpleLucidTester("!10", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("~b101", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    @Test
    fun testNegate() {
        var test = SimpleLucidTester("-20", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("--20", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    @Test
    fun testFunctions() {
        var test = SimpleLucidTester("\$signed(20)", ParseStage.ModuleInternals)
        var tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$signed(-20)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$clog2(7)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$clog2(0)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$clog2(1)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$clog2(129)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$pow(3,0)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$pow(2,4)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$reverse(b1100)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$reverse({b1100, b0011})", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$flatten(b1100)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$flatten({b1100, b0011})", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$build(b1100, 2)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$build(b11001001, 2, 2)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$unsigned(20)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$unsigned(\$signed(-20))", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$cdiv(8, 3)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$cdiv(9, 3)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$cdiv(10, 3)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$resize(8, 3)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
        ...
        test = SimpleLucidTester("\$resize(1, 3)", ParseStage.ModuleInternals)
        tree = test.expr().also { test.context.walk(it) }
    ...
    fun simpleSignalTest() {
        ...
        val test =
            SimpleLucidTester("test[2]", ParseStage.ModuleInternals, localSignalResolver = TestSignalResolver(signal))
    ...
    fun rangeSignalTest() {
        ...
        val test =
            SimpleLucidTester("test[2:1]", ParseStage.ModuleInternals, localSignalResolver = TestSignalResolver(signal))
        val exprCtx = test.expr().also { test.context.walk(it) }
    ...
    fun rangeOutOfBoundsSignalTest() {
        val signal =
            Signal("test", SignalDirection.Both, null, BitListValue("1010", 2, constant = false, signed = false))
        val test =
            SimpleLucidTester("test[9:1]", ParseStage.ModuleInternals, localSignalResolver = TestSignalResolver(signal))
        test.expr().also { test.context.walk(it) }
